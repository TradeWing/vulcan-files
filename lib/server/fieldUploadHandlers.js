...
export const uploadFromField = async (getValue, FSCollection, fieldValue) => {
...
export const createHandler = (fieldName, FSCollection, getValue) =>
  async function createHandler({ document, currentUser }) {
...
export const updateHandlerMultiple = (fieldName, FSCollection, getValue) =>
  ...
  async function updateHandler({ data, document, currentUser, oldDocument }) {
    ...
    if (
      !(
        isEmpty(fieldValues) ||
        some(fieldValues, fieldValue => hasUploadedFile(fieldValue))
      )
    ) {
      return document[fieldName]; // no change or deletion
    }
    // remove old files
    if (oldDocument[fieldName]) {
      const oldFileIds = oldDocument[fieldName];
      await Promise.all(oldFileIds.map(id => FSCollection.remove({ _id: id })));
    }
    // create files
    if (fieldValues) {
      if (Array.isArray(fieldValues)) {
